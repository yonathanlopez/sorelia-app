...
const SCAN_STEPS = [
  ...
  { key: 'trips', label: 'Found trips & travel', emoji: '✈️' },
  { key: 'events', label: 'Found upcoming events', emoji: '📅' },
  { key: 'birthdays', label: 'Found birthdays & anniversaries', emoji: '🎂' },
  { key: 'reminders', label: 'Found reminders & deadlines', emoji: '⏰' },
  { key: 'life_events', label: 'Found life events', emoji: '🌟' },
  { key: 'building', label: 'Saving your memories...', emoji: '✨' },
];
...
export default function GmailScannerModal({ onClose, onComplete }) {
  ...
  async function startScan() {
    ...
    try {
      ...
      setMemoriesCreated(res.data?.memories_created || 0);
      setPhase('done');
    } catch (e) {
      clearInterval(stepTimer);
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Sorelia reads your emails and extracts what matters</p>
            </div>
          </div>
        ...
        {phase === 'idle' && (
          <div>
            <p className="text-sm text-gray-600 mb-5 leading-relaxed">
              Sorelia reads the full content of your emails to extract meaningful memories — upcoming trips, important dates, appointments, and life events.
            </p>
            <div className="space-y-2.5 mb-6">
              {['✈️ Trips & travel plans from confirmations', '📅 Appointments & upcoming events', '🎂 Birthdays & anniversaries', '⏰ Deadlines & reminders', '🌟 Life milestones & events'].map((item, i) => (
            ...
            <button
              onClick={connected ? startScan : handleConnect}
            ...
            >
              ...
              {connected ? 'Re-scan Gmail' : 'Connect Gmail'}
            </button>
          </div>
        )}
        ...
        {phase === 'scanning' && (
          <div className="py-4">
            <div className="flex items-center gap-2 mb-6">
              ...
              <p className="text-sm font-medium text-violet-700">Reading your emails deeply...</p>
            </div>
            <div className="space-y-3">
              {SCAN_STEPS.map((step, i) => {
                ...
                return (
                  <div key={step.key} className={`flex items-center gap-3 transition-all duration-500 ${active || done ? 'opacity-100' : 'opacity-30'}`}>
                    {done ? (
                    ...
                    <span className={`text-sm ${done ? 'text-emerald-700 font-medium' : active ? 'text-violet-700 font-medium' : 'text-gray-400'}`}>
                      {step.emoji && done ? `${step.emoji} ${step.label}` : step.label}
                    </span>
                  </div>
                );
              })}
        ...
        {phase === 'done' && (
          <div className="py-2">
            <div className="text-center mb-6">
              ...
              <h3 className="font-bold text-gray-900 text-lg">{memoriesCreated} memories saved ✨</h3>
              <p className="text-sm text-gray-500 mt-1">Sorelia read your emails and remembered what matters</p>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {[
                ...
                { label: 'Reminders', value: stats?.reminders, emoji: '⏰' },
              ].map(item => (
